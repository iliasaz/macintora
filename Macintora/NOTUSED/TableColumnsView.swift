...
struct TableColumnsView: NSViewRepresentable {
    @Environment(\.colorScheme) var colorScheme
    @State var rows: [DBCacheTableColumn]
    var columnLabels = ["columnID", "columnName", "dataType", "dataTypeMod", "dataTypeOwner", "length", "precision", "scale", "isNullable", "numNulls", "numDistinct", "isIdentity", "isHidden", "isVirtual", "isSysGen", "defaultValue"]
    var autoColWidth = true
    
    init(columns: [DBCacheTableColumn]) {
        _rows = State(initialValue: columns.sorted { ($0.isHidden ? 1001 : $0.columnID) < ( $1.isHidden ? 1001 : $1.columnID) })
    }
    ...
    mutating func sort(by colName: String?, ascending: Bool) {
        ...
        let sortFn: (DBCacheTableColumn, DBCacheTableColumn) -> Bool
        switch sampleValue {
            case is String:
                sortFn = {
                    (lhs:DBCacheTableColumn , rhs: DBCacheTableColumn) in
    ...
    func makeCoordinator() -> TableColumnsViewCoordinator {
        TableColumnsViewCoordinator(self)
    }
    
    func makeNSView(context: Context) -> NSScrollView {
        log.trace("in makeNSView")
    ...
    func updateNSView(_ nsView: NSScrollView, context: Context) {
        log.trace("in updateNSView")
        let tableView = (nsView.documentView as! TableViewWithPasteboard)
        let coordinator = tableView.delegate as! TableColumnsViewCoordinator
        tableView.beginUpdates()
        tableView.reloadData()
        log.trace("reload complete, row count: \(tableView.numberOfRows)")
        coordinator.populateColumnHeaders()
        tableView.endUpdates()
        log.trace("populateColumnHeaders complete")
    }
}

class TableColumnsViewCoordinator: NSObject, NSTableViewDelegate, NSTableViewDataSource {
    var parent: TableColumnsView
    
    weak var tableView: TableViewWithPasteboard?
    
    init(_ parent: TableColumnsView) {
        self.parent = parent
    }
    ...
    func populateColumnHeaders() {
        ...
        if didColumnsChange() {
            // updating columns
            log.trace("in populateColumnHeaders, columns changed, old columns: \( (tableView.tableColumns.map {$0.title}).joined(separator: ",") )")
            for c in tableView.tableColumns {
                tableView.removeTableColumn(c)
            }
            tableView.columnWidths.removeAll()
            log.trace("columns removed")
            ...
            log.trace("columns added: \(tableView.tableColumns.count)")
        }
        log.trace("in populateColumnHeaders, columns NOT changed, columns:  \( (tableView.tableColumns.map {$0.title}).joined(separator: ",") )")
        log.trace("auto flag: \(self.parent.autoColWidth)")
        if parent.autoColWidth && tableView.numberOfRows > 0 {
            for (idx, _) in tableView.tableColumns.enumerated() {
                sizeToFit(column: idx)
            }
            log.trace("new column widths: \(tableView.columnWidths)")
        }
        log.trace("exiting populateColumnHeaders")
    }
    ...
    func sizeToFit(column: Int) {
        ...
        if let view = tableView.view(atColumn: column, row: 0, makeIfNecessary: true) as? NSTableCellView {
            ...
            for row in parent.rows {
                ...
                log.trace("col \(column) original width: \(tableView.tableColumns[column].width), new width: \(max(width, size.width))")
    ...
    func numberOfRows(in tableView: NSTableView) -> Int {
        log.trace("numberOfRows: \(self.parent.rows.count)")
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        if let existingCell = tableView.makeView(withIdentifier: cellIdentifier, owner: self) {
            ...
            switch cellIdentifier.rawValue {
                case "isNullable", "isHidden", "isIdentity", "isUserGenerated", "isVirtual":
...
//        let coordinator = self.delegate as! TableColumnsViewCoordinator
...
//struct TableColumnsView_Previews: PreviewProvider {
